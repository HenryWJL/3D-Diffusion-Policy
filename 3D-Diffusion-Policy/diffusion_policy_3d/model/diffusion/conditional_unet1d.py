...
class ConditionalUnet1DwIndex(nn.Module):
    def __init__(self, 
        input_dim,
        local_cond_dim=None,
        global_cond_dim=None,
    ...
    def forward(self, 
            sample: torch.Tensor, 
            timestep: Union[torch.Tensor, float, int],
            index: Union[torch.Tensor, float, int], 
            local_cond=None, global_cond=None, **kwargs):
        """
        x: (B,T,input_dim)
        timestep: (B,) or int, diffusion step
        ...
        if global_cond is not None:
            if self.condition_type == 'cross_attention':
                timestep_embed = timestep_embed.unsqueeze(1).expand(-1, global_cond.shape[1], -1)
            global_feature = torch.cat([timestep_embed, index_embed, global_cond], axis=-1)
...
#         super().__init__()
#         self.gate_conv = nn.Conv1d(in_channels, in_channels, 1)
...
class SpectralGate(nn.Module):
    ...
    def __init__(self, in_channels: int, cond_dim: int):
        super().__init__()
        self.gate_conv1 = nn.Conv1d(in_channels, in_channels, 2, 2, 1)
        self.gate_conv2 = nn.Conv1d(in_channels, in_channels, 1)
        # Maps conditioning to FiLM parameters
        self.film = nn.Linear(cond_dim, 2 * in_channels)
    ...
    def forward(self, x: torch.Tensor, cond: torch.Tensor):
        ...
        # FiLM modulation
        gate_score = gamma * gate_score + beta
        gate_score = self.gate_conv2(gate_score)
        gate_score = torch.sigmoid(gate_score)
        # FFT
...
#     """
#     1D Discrete Wavelet Transform (Downsampling)
...
#     """
#     1D Inverse Discrete Wavelet Transform (Upsampling)
...
#     def forward(self, x):
#         # Split channels back into low and high frequency components
#         xl, xh = x.chunk(2, dim=1)
...
#             down_modules.append(nn.ModuleList([
#                 ConditionalResidualBlock1D(
#                     dim_in if is_first else dim_in * 2, dim_out, cond_dim=cond_dim, 
#                     kernel_size=kernel_size, n_groups=n_groups,
#                     condition_type=condition_type),
#                 ConditionalResidualBlock1D(
#                     dim_out, dim_out, cond_dim=cond_dim, 
#                     kernel_size=kernel_size, n_groups=n_groups,
#                     condition_type=condition_type),
#                 DWT1D() if not is_last else nn.Identity()
#             ]))
...
#                     condition_type=condition_type),
#                 ConditionalResidualBlock1D(
#                     dim_in * 2, dim_in * 2, cond_dim=cond_dim,
#                     kernel_size=kernel_size, n_groups=n_groups,
#                     condition_type=condition_type),
#                 IDWT1D() if not is_last else nn.Identity()
#             ]))
...
#     """
#     1D Discrete Wavelet Transform (Downsampling)
#     Input: (B, C, T) -> Output: (B, C, T/2), (B, C, T/2)
#     """
#     def __init__(self):
#         super().__init__()
#         self.dwt = DWT1DForward(wave='haar', J=1)
...
#     """
#     1D Inverse Discrete Wavelet Transform (Upsampling)
#     Input: (B, C, T/2), (B, C, T/2) -> Output: (B, C, T)
#     """
#     def __init__(self):
#         super().__init__()
#         self.idwt = DWT1DInverse(wave='haar')
...
#             up_modules.append(nn.ModuleList([
#                 ConditionalResidualBlock1D(
#                     dim_out + dim_in, dim_in * 2, cond_dim=cond_dim,
#                     kernel_size=kernel_size, n_groups=n_groups,
#                     condition_type=condition_type),
#                 ConditionalResidualBlock1D(
#                     dim_in * 2, dim_in * 2, cond_dim=cond_dim,
