...
class RobosuiteRunner(BaseRunner):
    ...
    def run(self, policy: BasePolicy):
        ...
        for episode_idx in tqdm.tqdm(range(self.eval_episodes), desc=f"Eval in Robosuite {self.task_name} Pointcloud Env",
            ...
            D = int(self.shape_meta['action']['shape'][0])
            all_time_actions = np.zeros((T, T + H, D), dtype=np.float32)
            all_time_scores = np.zeros_like(all_time_actions)
            ...
            while not done:
                ...
                with torch.no_grad():
                    ...
                    score = policy.compute_score(action_pred)
                # device_transfer
                np_action = dict_apply(action,
                                            lambda x: x.detach().to('cpu').numpy())
                action = np_action_dict['action'].squeeze(0)
                # step env
                obs, reward, done, info = self.env.step(action)
